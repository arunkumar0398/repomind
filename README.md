...
RepoMind is not keyword search. Cognee's `remember` pipeline builds graph-vector memory from issue text, labels, modules, exception names, API paths, and comments. That lets `recall` surface conceptually related issues even when the query wording differs from the original issue.
...
If GitHub or Cognee is unavailable, RepoMind still uses `src/data/seed-issues.json` as a first-class demo fallback.
...
9. Click thumbs-down on a weak evidence card; show "Memory updated; re-running triage," then refresh the brief from a new Cognee recall.
